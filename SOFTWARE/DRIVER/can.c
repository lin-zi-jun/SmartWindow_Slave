...
u8 CAN_Mode_Init(u8 tsjw,u8 tbs2,u8 tbs1,u16 brp,u8 mode)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB | RCC_APB2Periph_AFIO, ENABLE);//使能PORTB AFIO时钟	                   											 
    ...
    GPIO_Init(GPIOB, &GPIO_InitStructure);		//初始化IO
 	...
 	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdMask; 
  	...
  	CAN_FilterInitStructure.CAN_FilterIdHigh=0x8640;////32位ID
...
//返回值:0,成功;
//		 其他,失败;
u8 Can_Send_DevInfo(unsigned char add,u8 data,u8*buf)
{	
  u8 mbox;
  u16 i=0;
  u8 j=0;
  CanTxMsg TxMessage;
  TxMessage.StdId=0xFF;							   //配置报文的标准标识符
  TxMessage.ExtId=add;						   //配置扩屏标识符
  TxMessage.IDE=CAN_ID_EXT;						   //报文使用标准标识符+扩屏标识符方式
  TxMessage.RTR=CAN_RTR_DATA;					   //报文为数据帧
  TxMessage.DLC=5;								   //报文发送数据长度为2字节
  TxMessage.Data[0]=data;
      for(j=1;j<5;j++)
   {
      TxMessage.Data[j]=buf[j-1];			               //要发送的数据第0字节    
   }      
...
// }
u8 Can_Send_Msg(unsigned char add,unsigned char data1,unsigned char data2)
{	
  u8 mbox;
  u16 i=0;
  CanTxMsg TxMessage;
